...
case "$cmd" in
  add|list|next|stop|trigger|due|mark-complete|mark-blocked) ;;
  *) usage >&2; exit 2 ;;
esac
...
python - "$cmd" "$format" "$id" "$skill" "$args" "$schedule" "$client" "$repo_path" "$runner_command" "$pipeline_id" "$message" "$one_shot" <<'PY'
import json
import re
import sys
...
if cmd == "add":
    ...
    runner = runner_command or ("codex exec" if client == "codex" else "")
