...
2. Config default: read `.codex/zskills-config.json` first, then
   `.claude/zskills-config.json`; stop before landing if both exist and
...
**Strip `pr`/`direct` from arguments**
...
Run only the operation that matches the next dangerous action:
...
If the helper is unavailable, perform the equivalent branch, cleanliness,
protected-main, and tracking checks inline and say that native hook fidelity is
degraded.

 before parsing issue numbers, focus,
...
If available, `every <SCHEDULE>` stores a `fix-issues` schedule using the remaining issue count/focus/flags as args; `next` runs `zskills-scheduler.sh next --skill fix-issues`; `stop` runs `zskills-scheduler.sh stop --skill fix-issues`; standalone `now` triggers `fix-issues` and runs due jobs. If unavailable, report unsupported without claiming background work.
