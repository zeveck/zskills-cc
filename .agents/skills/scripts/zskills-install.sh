...
usage() {
  cat <<'EOF'
Usage:
  zskills-install.sh [--client codex|claude|both|auto] [--project-root PATH] [--codex-scope project|global] [--codex-home PATH] [--upstream PATH] [--mirror-config] [--replace-all]

Installs generated Codex ZSkills to the project .agents directory by default,
or to the user .agents directory only when --codex-scope global is passed. Installs
upstream-clean Claude ZSkills to a project .claude directory. Project config is
intentionally client-scoped: .codex/zskills-config.json for Codex,
.claude/zskills-config.json for Claude.
...
project_root=""
codex_scope="project"
codex_home="${CODEX_SKILLS_HOME:-$HOME/.agents}"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --codex-scope) codex_scope="${2:-}"; shift 2 ;;
...
case "$codex_scope" in
  project|global) ;;
  *) echo "ERROR: invalid --codex-scope: $codex_scope" >&2; exit 2 ;;
esac
...
install_codex() {
  ...
  "$PYTHON_BIN" "$repo_root/scripts/generate-codex-skills.py" \
    --client codex \
    --upstream "$upstream" \
  ...
  sync_generated_entries "$tmp/codex-skills" "$codex_skills_dir" all
  if [ -d "$tmp/codex-skills/scripts" ]; then
    rm -rf "$codex_skills_dir/scripts"
    cp -a "$tmp/codex-skills/scripts" "$codex_skills_dir/"
  fi
  ensure_config "$project_root/.codex/zskills-config.json" "$project_root/.claude/zskills-config.json"
  echo "Installed Codex ZSkills to $codex_skills_dir"
}
