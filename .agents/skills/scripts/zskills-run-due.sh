...
[ -x "$scheduler" ] || scheduler="$root/.agents/skills/scripts/zskills-scheduler.sh"
[ -x "$scheduler" ] || scheduler="$HOME/.agents/skills/scripts/zskills-scheduler.sh"
...
"$PYTHON_BIN" - "$root" "$dry_run" "$scheduler" "$due_json" <<'PY'
import json
import os
import shlex
import shutil
...
- If you cannot complete the scheduled turn, exit non-zero. If you cannot force a non-zero exit, start the final response with `Blocked:` so the scheduler can mark the job blocked.
"""
...
for job in jobs:
    ...
    timeout = int(os.environ.get("ZSKILLS_RUNNER_TIMEOUT_SECONDS", "900"))
    try:
        proc = subprocess.run(
            cmd,
            cwd=repo,
        ...
        log_path.write_text(partial + f"\nTIMEOUT: runner exceeded {timeout}s\n")
        mark("mark-blocked", job_id, f"runner timed out after {timeout}s; log={log_path}")
        continue
    log_path.write_text(proc.stdout)
    blocked_text = "\nBlocked:" in proc.stdout or proc.stdout.strip().startswith("Blocked:")
    if proc.returncode == 0 and not blocked_text:
        mark("mark-complete", job_id, f"completed; log={log_path}")
    elif blocked_text:
        mark("mark-blocked", job_id, f"runner reported blocked; log={log_path}")
    else:
